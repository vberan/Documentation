Stored Procedure Vault.uspLoadCRMdboPhonecall {#spVaultuspLoadCRMdboPhonecall}
---------------------------------------------

Definition of Stored procedure:

``` {.sql}
CREATE PROCEDURE [Vault].[uspLoadCRMdboPhonecall] 
    @ThreadLogId int
        , @DataSourceId int
    AS
    BEGIN TRY
    ...
    -- log the initial table state
        UPDATE Etl.ThreadLog
            SET TableInitialRowCount = (SELECT COUNT(*) FROM [Vault].[CRMdboPhonecall] WITH (NOLOCK))
        WHERE ThreadLogId = @ThreadLogId;

            BEGIN TRANSACTION;

    
        MERGE INTO [Vault].[CRMdboPhonecall] AS v
        USING [Stage].[CRMdboPhonecall] AS s
     ...
     AND v.[ThreadLogDeleteId] = -1 AND v.[ThreadLogUpdateId] = -1 AND v.[ChangeOperation] <> N'D'
        ...
        OR v.[leftvoicemail] <> s.[leftvoicemail]
            OR (v.[leftvoicemail] IS NULL AND s.[leftvoicemail] IS NOT NULL)
        OR (v.[leftvoicemail] IS NOT NULL AND s.[leftvoicemail] IS NULL)
        ...
        OR v.[phonenumber] <> s.[phonenumber]
            OR (v.[phonenumber] IS NULL AND s.[phonenumber] IS NOT NULL)
        OR (v.[phonenumber] IS NOT NULL AND s.[phonenumber] IS NULL)
    ...
    INSERT INTO [Vault].[CRMdboPhonecall] (
            [Id]
            , [activityadditionalparams]
            , [activityid]
            , [activitytypecode]
    ...
    SELECT
            ...
            , -1
            , N'U' -- update
    FROM Stage.CRMdboPhonecall AS s
    INNER JOIN Vault.CRMdboPhonecall AS v
    ...
    SELECT
            ...
            , -1
            , N'D' -- update
    FROM Vault.CRMdboPhonecall AS v
    ...
    ;
        ...
        UPDATE Etl.ThreadLog
            SET
                ...
                , TableFinalRowCount = (SELECT COUNT(*) FROM [Vault].[CRMdboPhonecall] WITH (NOLOCK))
                , EndTime = GETDATE()
                , StatusId = 3      -- Finished
            WHERE ThreadLogId = @ThreadLogId;
