Stored Procedure Vault.uspLoadCRMdboOptionSetMetadata {#spVaultuspLoadCRMdboOptionSetMetadata}
-----------------------------------------------------

Definition of Stored procedure:

``` {.sql}
CREATE PROCEDURE [Vault].[uspLoadCRMdboOptionSetMetadata] 
    @ThreadLogId int
        , @DataSourceId int
    AS
    BEGIN TRY
    ...
    -- log the initial table state
        UPDATE Etl.ThreadLog
            SET TableInitialRowCount = (SELECT COUNT(*) FROM [Vault].[CRMdboOptionSetMetadata] WITH (NOLOCK))
        WHERE ThreadLogId = @ThreadLogId;

            BEGIN TRANSACTION;

    
        MERGE INTO [Vault].[CRMdboOptionSetMetadata] AS v
        USING [Stage].[CRMdboOptionSetMetadata] AS s
            ON ( s.[EntityName] = v.[EntityName] AND s.[IsUserLocalizedLabel] = v.[IsUserLocalizedLabel] AND s.[LocalizedLabelLanguageCode] = v.[LocalizedLabelLanguageCode] AND s.[Option] = v.[Option] AND s.[OptionSetName] = v.[OptionSetName]AND s.[DataSourceId] = v.[DataSourceId]
    ...
    INSERT INTO [Vault].[CRMdboOptionSetMetadata] (
            [EntityName]
            , [IsUserLocalizedLabel]
            , [LocalizedLabelLanguageCode]
            , [Option]
    ...
    SELECT
            ...
            , -1
            , N'U' -- update
    FROM Stage.CRMdboOptionSetMetadata AS s
    INNER JOIN Vault.CRMdboOptionSetMetadata AS v
        ON  s.[EntityName] = v.[EntityName] AND s.[IsUserLocalizedLabel] = v.[IsUserLocalizedLabel] AND s.[LocalizedLabelLanguageCode] = v.[LocalizedLabelLanguageCode] AND s.[Option] = v.[Option] AND s.[OptionSetName] = v.[OptionSetName] AND v.ThreadLogUpdateId = @ThreadLogId
    ...
    SELECT
            ...
            , -1
            , N'D' -- update
    FROM Vault.CRMdboOptionSetMetadata AS v
    ...
    ;
        ...
        UPDATE Etl.ThreadLog
            SET
                ...
                , TableFinalRowCount = (SELECT COUNT(*) FROM [Vault].[CRMdboOptionSetMetadata] WITH (NOLOCK))
                , EndTime = GETDATE()
                , StatusId = 3      -- Finished
            WHERE ThreadLogId = @ThreadLogId;
