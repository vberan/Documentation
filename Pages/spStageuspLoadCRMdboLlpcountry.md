Stored Procedure Stage.uspLoadCRMdboLlp\_country {#spStageuspLoadCRMdboLlpcountry}
------------------------------------------------

Definition of Stored procedure:

``` {.sql}
CREATE PROCEDURE [Stage].[uspLoadCRMdboLlp_country] 
    @ThreadLogId int
        , @DataSourceId int
    AS
    BEGIN TRY
        ...
        TRUNCATE TABLE [Stage].[CRMdboLlp_country]; 

        -- log the initial table state
        UPDATE Etl.ThreadLog
            SET TableInitialRowCount = (SELECT COUNT(*) FROM [Stage].[CRMdboLlp_country] WITH (NOLOCK))
        WHERE ThreadLogId = @ThreadLogId;

        DECLARE @loadDateFrom datetime2;
        SELECT @loadDateFrom = '1901-01-01';

        INSERT INTO [Stage].[CRMdboLlp_country]
            ( 
                [createdby]
        ...
        FROM [scaniacrmexport.database.windows.net].[ScaniaCRMExport].[dbo].[llp_country] AS s WITH (NOLOCK)

        --Log ThreadLog Success
        UPDATE Etl.ThreadLog
        SET ExtractRowCount = (SELECT COUNT(*) FROM [Stage].[CRMdboLlp_country] WITH (NOLOCK) WHERE ThreadLogId = @ThreadLogId)
            , InsertRowCount = (SELECT COUNT(*) FROM [Stage].[CRMdboLlp_country] WITH (NOLOCK) WHERE ThreadLogId = @ThreadLogId)
            , TableFinalRowCount = (SELECT COUNT(*) FROM [Stage].[CRMdboLlp_country] WITH (NOLOCK) )
            , EndTime = GETDATE()
